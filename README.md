# Differential methylation analysis pipeline for long-read (ONT) data
...
# Visualisation:
It is always a good idea to look at the identified DMRs. This pipeline can plot significant DMRs using [modbamtools](https://github.com/rrazaghi/modbamtools) - which is the default tool, and [methylartist](https://github.com/adamewing/methylartist) which can be triggered by providing a reference genome using `--reference`. Note that only `5mC` dmrs can be plotted at the moment (other bases will be added in the future). It supports haplotype-specific DMR plotting (by providing haplotagged modified bam files using `--phased_modBam` ) or DMRs between two samples (by providing modified bam files using `--input_modBam1` and `--input_modBam2`) but does not support plotting DMRs from group analysis. 

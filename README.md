...
# Visualisation:
It is always a good idea to look at the identified DMRs. This pipeline can plot significant DMRs using [modbamtools](https://github.com/rrazaghi/modbamtools) and [methylartist](https://github.com/adamewing/methylartist), which are provided as docker containers together with the pipeline. Note that only `5mC` dmrs can be plotted at the moment (other bases will be added in the future). It supports haplotype-specific DMR plotting (by providing haplotagged modified bam files using `--phased_modBam` ) or DMRs between two samples (by providing modified bam files using `--input_modBam1` and `--input_modBam2`) but does not support plotting DMRs from group analysis. 

You can provide a gene list with `--gene_list` to only plot significant DMRs for the provided genes (if present), or `--imprinted` to plot significant DMRs overlapping imprinted genes. Note that if you don't provide a gene list or use `--imprinted`, the pipeline will attempt to plot all significant DMRs. By default, the pipeline plots DMRs using [modbamtools](https://github.com/rrazaghi/modbamtools); to add methylartist plots or use `--methylartist_only`, `--reference` must be provided. 

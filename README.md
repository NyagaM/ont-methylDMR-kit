...
There are four docker images available from [DockerHub](https://hub.docker.com/repository/docker/nyagam/ont-methyldmr-kit/general); [DockerHub](https://hub.docker.com/repository/docker/nyagam/modbamtools/general); [DockerHub](https://hub.docker.com/repository/docker/nyagam/methylartist/tags); and [DockerHub](https://hub.docker.com/repository/docker/nyagam/seaborn/general) that contains all the tools/softwares required by the pipeline, making results highly reproducible. 

# Modkit pileup:
Summary counts of modified and unmodified bases from modified-base bam files (modbams) across reference genomic position is perfomed using [modkit](https://github.com/nanoporetech/modkit) and stored as a bedMethyl file. Users only need to provide modbams as either two modbams for analysis between two samples `(--input_modBam1 and --input_modBam2)` or a phased modbam for haplotype-specific analysis `(--phased_modBam)` and `--pileup` flag as a starting point to trigger modkit analysis and subsequent processes. See the full usage below. To compute high-confidence methylation calls, modkit `--filter_threshold` is automatically set based on the provided modbams using `modkit sample-probs -p 0.1`; meaning that the workflow excludes calls that fall in the lower 10th percentile - [see detailed explanation here](https://github.com/nanoporetech/modkit/blob/master/book/src/filtering.md). --pileup currently supports only 5-methylcytosine as `--5mC or --phased_mC` and N6-methyladenine as `--6mA or --phased_mA`.

# DMR analysis:
DMR analysis is performed using the R-package [DSS](https://bioconductor.org/packages/release/bioc/vignettes/DSS/inst/doc/DSS.html). It supports calling DMRs across 5-methylcytosine (`--5mC`), 5-hydroxymethylcytosine (`--5hmC`), N6-methyladenine (`--6mA`), and N4-methylcytosine (`--4mC`) modified bases. It also supports haplotype-specific DMRs using `--phased_mC`, `--phased_mA`, and `--phased_hmC`. Users can also provide raw bedmethyl files as generated by [modkit](https://github.com/nanoporetech/modkit) as a starting point using either `--input_file1` and `--input_file2` for calling DMRs between haplotypes or two samples; or `--input_group1` and `--input_group2` for group analysis (bedmethyls in these two folders must have `*.bed` extension). Note that `--phased_mC` and
...
# Visualisation:
It is always a good idea to look at the identified DMRs. This pipeline can plot significant DMRs using [modbamtools](https://github.com/rrazaghi/modbamtools), and [methylartist](https://github.com/adamewing/methylartist); which can be triggered by `--plot` and providing a reference genome using `--reference` (only useful for methylartist). Note that only `5mC` dmrs can be plotted at the moment (other bases will be added in the future). It supports haplotype-specific DMR plotting (by providing haplotagged modified bam files using `--phased_modBam` ) or DMRs between two samples (by providing modified bam files using `--input_modBam1` and `--input_modBam2`) but does not support plotting DMRs from group analysis. 

You can provide a gene list with `--gene_list` to only plot significant DMRs for the provided genes (if present), or `--imprinted` to plot significant DMRs overlapping imprinted genes. Note that if you don't provide a gene list or use `--imprinted`, the pipeline will attempt to plot all significant DMRs. 

There is a plots only mode triggered by the flag `--plots_only`, which requires an annotated DMR bed file `--annotated_dmrs` (generated by `process annotate_dmrs` ), modified bam files, and either `--phased_mC`, `--phased_hmC` for phased; or `--5mC`, `--4mC` for two-sample analysis. To use `--methylartist_only`, `--reference` must be provided.
...
Note: If running docker on a Mac with Apple Silicon chip, use `export DOCKER_DEFAULT_PLATFORM=linux/amd64 && nextflow run /path/to/ont-methylDMR-kit/ -profile docker`
```
```bash
